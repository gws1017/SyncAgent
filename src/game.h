...
struct Hero {
    ...
    long long lastHealAmount = 0; // 이번 틱 실제로 HP에 반영된 회복량 (정수, 저장 안 함)
    ...
    // 이번 틱에 "실제로 계산된" 흡혈량(정수 반영 전, 소숫점 포함) — 팝업 표시용.
    // 매 틱 흡혈이 작동하고 있다는 걸 보여주기 위해 0이어도 값을 남겨둠.
    float     lastHealFrac   = 0.0f;
